...
class Genesis_eNews_Updates extends WP_Widget {
	...
	function Genesis_eNews_Updates() {
		$widget_ops = array( 'classname' => 'enews-widget enews', 'description' => __('Displays Feedburner email subscribe form', 'genesis') );
	...
	function widget($args, $instance) {
		...
		echo $before_widget;
			...
			<form id="subscribe" action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo esc_js( $instance['id'] ); ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true"><input type="text" value="<?php echo $instance['input_text'] ?>" id="subbox" onfocus="if (this.value == '<?php echo $instance['input_text'] ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php echo $instance['input_text'] ?>';}" name="email"/><input type="hidden" value="<?php echo esc_attr( $instance['id'] ); ?>" name="uri"/><input type="hidden" name="loc" value="en_US"/><input type="submit" value="<?php echo $instance['button_text']; ?>" id="subbutton" /></form>
			<?php }
