...
function genesis_update_check() {
	...
	if ( !$genesis_update ) {
		...
		$genesis_update = wp_remote_retrieve_body(wp_remote_post($url, $options));
...
function genesis_upgraded_notice() {
	...
	if ( isset($_REQUEST['upgraded']) && $_REQUEST['upgraded'] == 'true') {
		echo '<div id="message" class="updated highlight" id="message"><p><strong>'.sprintf(__('Congratulations! You successfully upgraded to Genesis %s', 'genesis'), genesis_get_option('theme_version')).'</strong></p></div>';
	}
...
function genesis_update_nag() {
	...
	if ( !current_user_can('update_themes') || !$genesis_update )
		return false;
	
	$update_url = wp_nonce_url('update.php?action=upgrade-theme&amp;theme=genesis', 'upgrade-theme_genesis');
	$update_onclick = esc_js( __('Upgrading Genesis will overwrite the current installed version of Genesis. Are you sure you want to upgrade?. "Cancel" to stop, "OK" to upgrade.', 'genesis') );
	
	echo '<div id="update-nag">';
	printf(__('Genesis %s is available. <a href="%s" class="thickbox thickbox-preview">Check out what\'s new</a> or <a href="%s" onclick="return genesis_confirm(\'%s\');">update now</a>.', 'genesis'), $genesis_update['new_version'], $genesis_update['changelog_url'], $update_url, $update_onclick);
	echo '</div>';
}
...
function genesis_update_email() {
	...
	// Let's send an email!
	$subject = __('Genesis ', 'genesis') . $update_check['new_version'] . __(' is available for ', 'genesis') . get_bloginfo('url');
	$message = __('Genesis ', 'genesis') . $update_check['new_version'] . __(' is now available. We have provided 1-click updates for this theme, so please log into your dashboard and update at your earliest convenience.', 'genesis') . "\n\n" . wp_login_url();
	
	wp_mail( sanitize_email($email), $subject, $message ); // send that puppy!
	
	// Update the option so we don't send emails on every pageload!
	delete_option('genesis-update-email');
	add_option('genesis-update-email', $update_check['new_version'], TRUE);
